...
Now we can see an ingress rule runnning for `www.external.com `.
...
Now lets check the service again
...
Now lets pass a header which matches a URL to one of the services we exposed in an earlier 
...
Now test if `web-two`` will work. This wont, as there are no ingress rules defined. So we will edit the existing ingress rule
...
Now edit the ingress rule to change 
